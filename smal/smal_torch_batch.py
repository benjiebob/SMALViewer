...
class SMALModel(Module):
  def __init__(self, device=torch.device('cuda'), model_path='smal/smal_CVPR2017_np.pkl'):
    
    super(SMALModel, self).__init__()
    with open(model_path, 'rb') as model_reader:
      u = pickle._Unpickler(model_reader)
      u.encoding = 'latin1'
      params = u.load()
      print (params.keys())
    ...
    self.faces = torch.from_numpy(params['f'].astype(np.int32)).type(torch.int32)
    self.num_faces = self.faces.shape[0]
    self.device = device if device is not None else torch.device('cpu')
    for name in ['J_regressor', 'weights', 'posedirs', 'v_template', 'shapedirs', 'faces']:
  ...
  def forward(self, betas, pose, trans, simplify=False):
    ...
    # J = torch.matmul(self.J_regressor[:24], v_shaped)
    J = torch.matmul(self.J_regressor[:33], v_shaped)
      ...
      self.pack(
        torch.matmul(
          ...
          torch.reshape(
            ...
            torch.cat((J, torch.zeros((batch_num, 33, 1), dtype=torch.float32).to(self.device)), dim=2),
            # (batch_num, 24, 4, 1)
            (batch_num, 33, 4, 1)
          )
        )
